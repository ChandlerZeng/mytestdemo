...
public class DemoDBManager {
    ...
    private DemoDBManager(){
        dbHelper = DbOpenHelper.getInstance(MyTestApplication.getInstance().getApplicationContext());
...
//                }
    ...
    synchronized public void saveRandomIds(List<String> randomIds){
        ...
        if(db.isOpen()){
            ...
            + UserDao.Random_IDS_TABLE_NAME+" where "+UserDao.COLUMN_NAME_RANDOM_IDS+"='"+ids+"'",null);
                if(cursor.moveToNext()){
                    String id = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_RANDOM_IDS));
                    LogUtil.e("id:"+id);
    ...
    synchronized public void deleteRandomId(String id){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            db.delete(UserDao.Random_IDS_TABLE_NAME, UserDao.COLUMN_NAME_RANDOM_AUTO_IDS + " = ?", new String[]{id});
        }
    }
    ...
    synchronized public List<RandomId> getRandomIds(){
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.Random_IDS_TABLE_NAME+" order by "
        +UserDao.COLUMN_NAME_RANDOM_AUTO_IDS+ " desc", null);
            while (cursor.moveToNext()) {
                RandomId randomId = new RandomId();
                String id = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_RANDOM_AUTO_IDS)); 
                String random_id = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_RANDOM_IDS)); 
                randomId.setId(id);
